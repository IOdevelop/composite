...
class academic_year(models.Model):
    ...
    month_ids = fields.One2many('academic.month', 'year_id', string='Semester',help="related Academic Semesters")
...
class academic_month(models.Model):
    ...
    #related_exam=   fields.Many2one('exam.exam',"Related exam for this academic semester")
...
#    cmp_id =        fields.related('school_id','company_id',relation="res.company", string="Company Name", type="many2one", store=True)
    ...
    def name_get(self):
        ...
        for standard in self:
            name = "StandardId"#standard.standard_id.name+"[" + standard.division_id.name + "]" 
...
class subject_subject(models.Model):
    ...
    code =          fields.Char('Code', required=True)
    department=     fields.Many2one('school.department',"Department")
    maximum_marks = fields.Integer("Maximum marks",default=100)
...
class school_campus(models.Model):
    ...
    name=fields.Char('Name')
    vc=fields.Many2one('hr.employee','Deputy Vice Chancellor')
...
class school_department(models.Model):
    ...
    name=fields.Char("Name",required=True)
    campus=fields.Many2one('school.campus',"Campus",required=True)
    hod=fields.Many2one('hr.employee','Head of Department')
...
class student_student(models.Model):
    ...
    course =            fields.Many2one('school.course','Course')
    ...
    school_njala=      fields.Many2one('school.school.njala','School',required=True)
...
class grade_line(models.Model):
    ...
    value=      fields.Integer("Interpretation",required=True,help="Interpretation of grade as used in Njala context eg. A=5,B=4,C=3 etc")
    to_mark =   fields.Integer('To Marks',required=True, help="The grade will ends to this marks.")
    grade =     fields.Char('Grading Sequence', required=True, help="Grading equence")
